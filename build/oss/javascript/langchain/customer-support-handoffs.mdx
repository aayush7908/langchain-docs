...
Unlike the [supervisor pattern](/oss/javascript/langchain/supervisor) where sub-agents are called as tools, handoffs create a **state machine** where the active step changes based on workflow progress. Each "step" is just a different configuration (system prompt + tools) of the same underlying agent, selected dynamically based on state.
