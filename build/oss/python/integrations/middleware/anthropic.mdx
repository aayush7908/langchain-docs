...
- **Tools** ([`langchain_anthropic.tools`](/oss/python/integrations/chat/anthropic#built-in-tools)): Low-level building blocks where you provide your own execution logic
...
| Server-side tools (web search, code execution) | Tools | Anthropic executes these server-side |
...
| System prompt injection | ✅ (memory) | ❌ |
...
def my_bash_execute(*, command=None, restart=False, **kwargs):
    """You implement the execution logic."""
    if restart:
        return "Session restarted"

    # Run command in your environment
    result = subprocess.run(command, shell=True, capture_output=True, text=True)
    return result.stdout + result.stderr

agent = create_agent(
    model=ChatAnthropic(model="claude-sonnet-4-5-20250929"),
    tools=[tools.bash_20250124(execute=my_bash_execute)],
)
```
