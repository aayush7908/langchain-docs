...
The recommended way to query runs (the span data in LangSmith traces) is to use the `list_runs` method in the SDK or `POST /runs/query` endpoint in the API.
...
<Tip>
    **TL;DR:** Use `<=7days` time window and avoid full-text search to reach highest rate-limit.
</Tip>

Here is the `POST /runs/query` endpoint rate limits. As a reminder, this endpoint is used by the [list_runs](https://docs.smith.langchain.com/reference/python/client/langsmith.client.Client#langsmith.client.Client.list_runs)/[listRuns](https://docs.smith.langchain.com/reference/js/classes/client.Client#listruns) SDKs methods.

### <Icon icon="face-smile" iconType="solid" color="#059669"/> Short time-window: `10requests/10sec`

Use `end_time - start_time <= 7 days`. We use query time if `end_time` isn’t provided.
...
client.listRuns({
    projectName: "<your_project>",
    startTime: new Date(Date.now() - 7 * ONE_DAY_MS),
})
```
</CodeGroup>

### <Icon icon="face-meh" iconType="solid" color="#ffc107"/> Larger time-windows: `3requests/10sec`

You will fall in this category if you don't provide a `start_time` or if `end_time - start_time > 7 days`. We use query time if `end_time` isn’t provided.

Reach higher limits by using short time windows and pagination.
